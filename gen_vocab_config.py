...
def evaluate(model, data_loader, forward_fn, metrics_fn, i2w, is_test=False):
    ...
    # report = classification_report(list_label, list_hyp, target_names=['anger', 'love', 'sadness', 'joy', 'fear'])
...
#         model.load_state_dict(torch.load(model_dir + "/best_model_" + str(i) + ".th"))
        ...
        # Evaluate
#         test_loss, test_metrics = evaluate(model, data_loader=test_loader, forward_fn=args['forward_fn'], metrics_fn=args['metrics_fn'], i2w=i2w, is_test=True)
